...
const isFieldValue = (value: any): value is FieldValue => {
  return value && typeof value.isEqual === 'function';
};
...
const exportSubscriptionsToExcel = (subscriptions: Subscription[], filename: string) => {
  const worksheetData = subscriptions.map(sub => ({
    ...
    'تاريخ البدء': isTimestamp(sub.startDate) ? format(sub.startDate.toDate(), 'yyyy-MM-dd') : format(new Date(sub.startDate), 'yyyy-MM-dd'),
    'تاريخ الانتهاء': isTimestamp(sub.endDate) ? format(sub.endDate.toDate(), 'yyyy-MM-dd') : format(new Date(sub.endDate), 'yyyy-MM-dd'),
    'السعر الإجمالي': sub.totalPrice,
    'الحالة': sub.status,
    'تاريخ الإنشاء': isTimestamp(sub.createdAt) ? format(sub.createdAt.toDate(), 'yyyy-MM-dd HH:mm') : format(new Date(sub.createdAt), 'yyyy-MM-dd HH:mm'),
  }));
...
const exportSubscriptionsToPdf = (subscriptions: Subscription[], filename: string) => {
  ...
  subscriptions.forEach(sub => {
    const subData = [
      sub.clientName,
      sub.tier,
      isTimestamp(sub.startDate) ? format(sub.startDate.toDate(), 'yyyy-MM-dd') : format(new Date(sub.startDate), 'yyyy-MM-dd'),
      isTimestamp(sub.endDate) ? format(sub.endDate.toDate(), 'yyyy-MM-dd') : format(new Date(sub.endDate), 'yyyy-MM-dd'),
...
type SubscriptionsState = {
  ...
  addSubscriptionAndTasks: (subscriptionData: Omit<Subscription, 'id' | 'tier' | 'createdAt' | 'status'> & { startDate: string; endDate: string; }, automaticTasks: Task[], manualTasks: Task[]) => Promise<string | null>;
...
export const useSubscriptionsStore = create<SubscriptionsState>((set, get) => ({
  ...
  fetchSubscriptions: async () => {
    ...
    try {
      ...
      const subsData = querySnapshot.docs.map(doc => ({ 
        id: doc.id, 
        ...doc.data(),
        startDate: isTimestamp(doc.data().startDate) ? doc.data().startDate.toDate().toISOString() : doc.data().startDate,
        endDate: isTimestamp(doc.data().endDate) ? doc.data().endDate.toDate().toISOString() : doc.data().endDate,
        createdAt: isTimestamp(doc.data().createdAt) ? doc.data().createdAt.toDate().toISOString() : doc.data().createdAt
      } as Subscription));
  ...
  addSubscriptionAndTasks: async (
  subscriptionData: Omit<Subscription, 'id' | 'tier' | 'createdAt' | 'status'> & {
    startDate: string;
    endDate: string;
  },
  automaticTasks: Task[],
  ...
  try {
    ...
    batch.set(subDocRef, newSubscription);
  ...
  updateSubscription: async (updatedSubscription) => {
    ...
    try {
      await setDoc(subDocRef, {
        ...updatedSubscription,
        startDate: isTimestamp(updatedSubscription.startDate) ? updatedSubscription.startDate : Timestamp.fromDate(new Date(updatedSubscription.startDate)),
        endDate: isTimestamp(updatedSubscription.endDate) ? updatedSubscription.endDate : Timestamp.fromDate(new Date(updatedSubscription.endDate)),
        createdAt: isTimestamp(updatedSubscription.createdAt) ? updatedSubscription.createdAt : serverTimestamp(),
  ...
  importSubscriptions: async (jsonData) => {
    ...
    try {
      for (const subData of jsonData) {
        ...
        batch.set(subDocRef, dataWithoutId);
      }
